...
InputParameters validParams<SeebeckEffect>()
{
  ...
  // params.addParam<MaterialPropertyName>("ecC", "Electrical Conductivity", "Property name of the electrical conductivity material property");
  params.addParam<MaterialPropertyName>("sbC", "Seebeck coefficient", "Property name of the Seebeck coefficient material property");
...
{
  return - _grad_test[_i][_qp](_component) * (_potential_E_int_grad[_qp](_component) + _sbC[_qp] * _T_grad[_qp](_component)) * _len_scale;
}

Real
SeebeckEffect::computeQpJacobian()
{
   // return - _grad_test[_i][_qp](_component) * (_potential_E_int_grad[_qp](_component) + _sbC[_qp] * _grad_phi[_j][_qp](_component)) * _len_scale;
   return - _grad_test[_i][_qp](_component) * (_grad_phi[_j][_qp](_component) + _sbC[_qp] * _T_grad[_qp](_component)) * _len_scale;
...
SeebeckEffect::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  //   }
    if(jvar == _T_var)
     {
      return (-_grad_test[_i][_qp](_component)) * _sbC[_qp] * _grad_phi[_j][_qp](_component) * _len_scale;
