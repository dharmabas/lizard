...
OhmsLaw::computeQpResidual()
{
  ...
  // Real Rcur = 0.0;
  return _test[_i][_qp] * _electrical_conductivity[_qp] * _potential_E_int_grad[_qp](_component) *
         _len_scale;
  ///  Moose::out << "\n R_cur-"; std::cout << " = " << Rcur;
  // return Rcur;
}
...
{
  return _test[_i][_qp] * _electrical_conductivity[_qp] * _grad_phi[_j][_qp](_component) *
         _len_scale;
}
